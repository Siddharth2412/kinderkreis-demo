...
def test_decline_notifies_parent(client, provider_profile, eltern, tagespflege):
    ...
    client.post(f"/api/bookings/{booking['id']}/decline", headers=auth_headers(tagespflege["token"]))
...
def test_mark_all_notifications_read(client, provider_profile, eltern, tagespflege):
    for _ in range(3):
        client.post(
            "/api/bookings",
            json={"provider_id": provider_profile["id"], **BOOKING_PAYLOAD, "child_name": "Kind"},
            headers=auth_headers(eltern["token"]),
        )
    assert client.get(
